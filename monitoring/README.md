...
Used to visualize time series metric data stored in Prometheus, and logs stored in Loki.
...
* Telegram alerting
  ...
  To get bot api token, use @bot_father on telegram. Any default newly generated bot should work. Add the bot to a group.

  To get group id, go `https://api.telegram.org/bot<api-token>/getUpdates`. Find:
...
Stores logs from log files or docker containers in a central log storage. Allows for easy querying using LogQL query language and label-based indexing. Logs on the services are generated using the Pino structured logging library (configured in `common/src/logger`).

# Deployment

Run `/home/sadm/hooks/monitoring_cd.sh` in the NUS server.
