...
export async function init() {
  ...
  bot.command('start', (ctx) => {
    ctx.reply(
      `your channel id is ${ctx.update.channel_post?.sender_chat.id}, please input this into your settings in trofos.`,
    );
  });
}
...
export async function sendToProject(projectId: number, message: string) {
  ...
  //TO FIX: Telegram bot is causing server to go down momentarily
