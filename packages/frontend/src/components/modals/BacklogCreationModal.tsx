...
function BacklogCreationModal({
  ...
  let sprintOptionsDescending = projectSprintData?.sprints ?
    projectSprintData.sprints.map((sprint) => {
  ...
  const shouldCreateNewSprint = retrospective &&
    sprintOptionsDescending[0]?.id === retrospective.sprint_id &&
    !projectSprintData?.sprints?.find((s) => s.status === 'current');

  const autoGeneratedNewSprint = projectSprintData?.sprints && projectSprintData.sprints.length > 0 ?
    autoSuggestNewSprint(projectSprintData.sprints[projectSprintData.sprints.length - 1]) : GENERIC_NEW_SPRINT;
